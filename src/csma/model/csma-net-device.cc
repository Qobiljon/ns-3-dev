...
namespace ns3 {
    ...
    void CsmaNetDevice::ControllerReceive(Ptr<Packet> packet, Ptr<CsmaNetDevice> senderDevice){
    ...
    uint8_t* LvapAction::GenerateActionMessage(uint8_t action, Mac48Address address, uint8_t channelNumber, bool nonErpStation, bool nonHtStation, Mac48Address lvapAddress){
        uint8_t* raw = new uint8_t[LvapAction::RAW_LENGTH];
        memset(raw, 0, LvapAction::RAW_LENGTH);
        uint8_t* ptr = raw;
        
        raw[0] = action; ptr++;
        address.CopyTo(ptr); ptr+=6;
        lvapAddress.CopyTo(ptr); ptr+=6;
        *ptr |= channelNumber << 2;
        *ptr |= nonErpStation << 1;
        *ptr |= nonHtStation;
    ...
    MessageStruct* LvapAction::RecoverActionMessage(uint8_t* raw, uint8_t payloadLength){
        NS_ASSERT(payloadLength == 64); // only ap<->controller management packets
        
        uint8_t* cursor = raw + 14;
        MessageStruct* res = new MessageStruct();
        res->action = *cursor; cursor++;
        res->address.CopyFrom(cursor); cursor+=6;
        res->lvapAddress.CopyFrom(cursor); cursor+=6;
        res->channelNumber = (*cursor) >> 2;
        res->nonErpStation = 2&(*cursor);
        res->nonHtStation = 1&(*cursor);
        return res;
    }
    // </editor-fold>
