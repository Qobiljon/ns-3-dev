...
namespace ns3 {
    ...
    class LvapAction{
    public:
        static const uint8_t RAW_LENGTH = 14; // the length of raw byte version of each action is constantly 13
        
        static const uint8_t RESULT_DONE = 0; // to notify that the previous action has been completed
        static const uint8_t RESULT_FAILED = 1; // to notify the failure in processing the previous action
        static const uint8_t ACTION_GET_LVAP_STATE = 10; // action to request info about the LVAP to handoff
        static const uint8_t ACTION_DETACH_LVAP = 11; // action to detach an LVAP from AP
        static const uint8_t ACTION_ATTACH_LVAP = 12; // action to attach an LVAP to AP
        static const uint8_t ACTION_CHOOSE_AP_FOR_STA = 13; // action to ask controller decide which ap should answer to probe request
        
        static uint8_t* GenerateActionMessage(uint8_t action, Mac48Address address, uint8_t channelNumber, bool nonErpStation, bool nonHtStation, Mac48Address lvapAddress);
        static MessageStruct* RecoverActionMessage(uint8_t* raw, uint8_t payloadLength);
    };
    // </editor-fold>
    ...
    public:
        // <editor-fold desc="LVAP">
        bool is_central_controller = false;
        
        void ControllerReceive (Ptr<Packet> p, Ptr<CsmaNetDevice> sender);
        // </editor-fold>
