...
namespace ns3 {
    ...
    void MacLow::NonApReceiveOk (Ptr<Packet> packet, double rxSnr, WifiTxVector txVector, bool ampduSubframe) {
    ...
    void MacLow::ApReceiveOk (Ptr<Packet> packet, double rxSnr, WifiTxVector txVector, bool ampduSubframe) {
        NS_LOG_FUNCTION (this << packet << rxSnr << txVector.GetMode () << txVector.GetPreambleType ());
        /* A packet is received from the PHY.
         * When we have handled this packet,
         * we handle any packet present in the
        ...
        if(hdr.IsAck()){
            for(std::map<Mac48Address, Mac48Address>::iterator iter = ap->sta_lvap_map.begin(); iter != ap->sta_lvap_map.end(); iter++)
                if(hdr.GetAddr1() == iter->second)
                    hdr.SetAddr1(m_self);
                else if(hdr.GetAddr2() == iter->second)
                    // our packet, so drop it dumbass
                    return;
    ...
    MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiTxVector txVector, bool ampduSubframe)
    {
        ...
        if(ApWifiMac::lvap_mode)
            if(ApWifiMac::ap_objects.find(m_self) == ApWifiMac::ap_objects.end())
                NonApReceiveOk(packet, rxSnr, txVector, ampduSubframe);
            else
                ApReceiveOk(packet, rxSnr, txVector, ampduSubframe);
        else
            NonApReceiveOk(packet, rxSnr, txVector, ampduSubframe);
        // </editor-fold>
    }
