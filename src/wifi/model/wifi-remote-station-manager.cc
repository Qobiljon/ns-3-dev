...
namespace ns3 {
    ...
    WifiRemoteStation*
    WifiRemoteStationManager::RemoveRemoteSta(Mac48Address address){
        const WifiRemoteStationManager* _this = const_cast<WifiRemoteStationManager *> (this);
        
        for (Stations::const_iterator sta_iter = _this->m_stations.begin(); sta_iter != _this->m_stations.end(); sta_iter++)
            if ((*sta_iter)->m_state->m_address == address){
                WifiRemoteStationState* remStaState = new WifiRemoteStationState();
                for (StationStates::const_iterator stt_iter = _this->m_states.begin(); stt_iter != _this->m_states.end(); stt_iter++)
                    if ((*stt_iter)->m_address == address){
                        remStaState->m_address = (*stt_iter)->m_address;
                        remStaState->m_aggregation = (*stt_iter)->m_aggregation;
                        remStaState->m_channelWidth = (*stt_iter)->m_channelWidth;
                        remStaState->m_greenfield = (*stt_iter)->m_greenfield;
                        remStaState->m_guardInterval = (*stt_iter)->m_guardInterval;
                        remStaState->m_heSupported = (*stt_iter)->m_heSupported;
                        remStaState->m_htSupported = (*stt_iter)->m_htSupported;
                        
                        remStaState->m_info = *new WifiRemoteStationInfo();
                        remStaState->m_info.m_failAvg = (*stt_iter)->m_info.m_failAvg;
                        remStaState->m_info.m_lastUpdate = (*stt_iter)->m_info.m_lastUpdate;
                        remStaState->m_info.m_memoryTime = (*stt_iter)->m_info.m_memoryTime;
                        
                        remStaState->m_ldpc = (*stt_iter)->m_ldpc;
                        remStaState->m_ness = (*stt_iter)->m_ness;
                        
                        remStaState->m_operationalMcsSet = *new WifiModeList();
                        for(WifiModeList::iterator wml_iter = (*stt_iter)->m_operationalMcsSet.begin(); wml_iter != (*stt_iter)->m_operationalMcsSet.end(); wml_iter++)
                            remStaState->m_operationalMcsSet.push_back(*wml_iter);
                        
                        remStaState->m_operationalRateSet = *new WifiModeList();
                        for(WifiModeList::iterator ops_iter = (*stt_iter)->m_operationalRateSet.begin(); ops_iter != (*stt_iter)->m_operationalRateSet.end(); ops_iter++)
                            remStaState->m_operationalRateSet.push_back(*ops_iter);
                        
                        remStaState->m_qosSupported = (*stt_iter)->m_qosSupported;
                        remStaState->m_shortGuardInterval = (*stt_iter)->m_shortGuardInterval;
                        remStaState->m_shortPreamble = (*stt_iter)->m_shortPreamble;
                        remStaState->m_shortSlotTime = (*stt_iter)->m_shortSlotTime;
                        remStaState->m_state = (*stt_iter)->m_state;
                        remStaState->m_stbc = (*stt_iter)->m_stbc;
                        remStaState->m_streams = (*stt_iter)->m_streams;
                        remStaState->m_vhtSupported = (*stt_iter)->m_vhtSupported;
                        
                        //delete (*stt_iter);
                        break;
                    }
                
                WifiRemoteStation* remSta = DoCreateStation();
                remSta->m_slrc = (*sta_iter)->m_slrc;
                remSta->m_ssrc = (*sta_iter)->m_ssrc;
